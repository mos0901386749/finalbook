...
const addBook = (book) => {
    ...
    alt="card image" />
    <div class="card-body" onclick="showbook">
...
const removeAllResult = () => {
    const removeAllResult = document.querySelector(".book");
...
const deleteBook = async (isbn) => {
  ...
  const showBook = (book) => {   
    const itemCard = document.createElement("p");
    item.className = "card";
    item.style = "max-width:20rem";
    const ShowCard = `
    <img class="card-img-top"
    ...
    `;
    item.innerHTML = ShowCard;
    const CardResult = document.querySelector(".card");
    CradResult.appendChild(itemCard);
